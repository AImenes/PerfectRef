from engine.classes.query import Query
from engine.classes.atom import Atom
from engine.classes.entry import Variable
from engine.classes.entry import Constant
...
onto = get_ontology("engine/ontologies/Test4.owl").load()

query_string = "q(?x) :- Professor(?x)^teachesTo(?x, ?y)"
query_obj = parse_query(query_string)
print("hello")
